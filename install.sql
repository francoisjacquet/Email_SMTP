...
INSERT INTO config (school_id, title, config_value)
SELECT sch.id, 'EMAIL_SMTP_ENCRYPTION', ''
FROM schools sch
WHERE NOT EXISTS (SELECT title
    FROM config
...
INSERT INTO config (school_id, title, config_value)
SELECT sch.id, 'EMAIL_SMTP_USERNAME', ''
FROM schools sch
WHERE NOT EXISTS (SELECT title
    FROM config
...
INSERT INTO config (school_id, title, config_value)
SELECT sch.id, 'EMAIL_SMTP_PASSWORD', ''
FROM schools sch
WHERE NOT EXISTS (SELECT title
    FROM config
...
INSERT INTO config (school_id, title, config_value)
SELECT sch.id, 'EMAIL_SMTP_FROM_NAME', ''
FROM schools sch
WHERE NOT EXISTS (SELECT title
    FROM config
